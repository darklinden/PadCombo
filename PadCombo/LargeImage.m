...
+ (void)test_save_image:(UIImage *)img
{
    ...
    int64_t i = 1;
    NSString *path_des = [NSString stringWithFormat:@"%@/%lld.png", path_doc, i];
    
    while ([[NSFileManager defaultManager] fileExistsAtPath:path_des]) {
        i++;
        path_des = [NSString stringWithFormat:@"%@/%lld.png", path_doc, i];
    }
    
    NSLog(@"test_save_image: %@", path_des.lastPathComponent);
